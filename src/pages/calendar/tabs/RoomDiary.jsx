...
export default function RoomDiary({diaries,schedules,isMulti,getName,getProfile,room,updateRoom,onSchClick,updateDiaryLikes,updateDiaryComments,userId,myRole}){
  ...
  return <div className="gr-thr-feed">
    ...
    {/* 좋아요 누른 사람 목록 팝업 */}
    {likesPopup&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:999,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setLikesPopup(null)}>
      <div style={{background:'var(--gr-bg)',borderRadius:16,padding:20,width:'90%',maxWidth:320,maxHeight:'60vh',overflow:'auto'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
          <div style={{fontSize:15,fontWeight:700}}>좋아요</div>
          <button className="gr-icon-btn" onClick={()=>setLikesPopup(null)}><I n="x" size={18}/></button>
        </div>
        {likesPopup.map(uid=>{
          const p = getProfile ? getProfile(uid) : {name:getName(uid),img:null};
          return <div key={uid} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:'1px solid var(--gr-brd)'}}>
            <Avatar name={p.name} size={36} src={p.img}/>
    ...
    {feed.map(item=>{
      ...
      const profile = getProfile ? getProfile(item.createdBy||userId) : {name:getName(item.createdBy||userId),img:null};
      const author = profile.name;
      const authorImg = profile.img;
      ...
      return <div key={(isSch?'s-':'')+item.id} className="gr-thr-post">
        ...
        <div className="gr-thr-post-header">
          <Avatar name={author} size={40} src={authorImg}/>
        ...
        <div className="gr-thr-post-actions">
          <button className="gr-thr-act-btn" onClick={e=>toggleLike(item,e)}>
            ...
            {likeCount>0&&<span style={{cursor:'pointer'}} onClick={e=>{e.stopPropagation();setLikesPopup(item.likes||[]);}}>{likeCount}</span>}
          </button>
        ...
        {isDiary && (replyTo===item.id || (item.comments||[]).length>0) && <div className="gr-thr-comments">
          {(item.comments||[]).map(c=>{
            const cp = getProfile ? getProfile(c.by) : {name:getName(c.by),img:null};
            return <div key={c.id} className="gr-thr-comment">
              <Avatar name={cp.name} size={28} src={cp.img}/>
              <div style={{flex:1,minWidth:0}}>
                <div className="gr-thr-comment-header">
                  <span className="gr-thr-comment-name">{cp.name}</span>
