...
function AppMain({ authUser, onLogout }){
  ...
  const renderSidebar = () => (
    <div className="gr-sidebar">
      ...
      {isWide && <div className="gr-btab"><button className={`gr-btab-btn ${tab==='friends'?'on':''}`} onClick={()=>{navigate('/profile');}}><I n="user" size={22}/><span>친구</span></button><button className={`gr-btab-btn ${tab==='rooms'?'on':''}`} onClick={()=>{const myRoom=rooms.find(r=>r.isPersonal);navigate(myRoom?`/calendar/${myRoom.id}/cal`:'/calendar');}}><I n="cal" size={22}/><span>캘린더</span></button><button className={`gr-btab-btn ${tab==='alarm'?'on':''}`} onClick={()=>{const now=Date.now();setLastReadAlarm(now);try{localStorage.setItem('gr_last_read_alarm',String(now));}catch(e){}navigate('/alarm');}}><div className="gr-btab-bell-wrap"><I n="bell" size={22}/>{unreadAlarmCount>0&&<span className="gr-btab-badge">{unreadAlarmCount>99?'99+':unreadAlarmCount}</span>}</div><span>알림</span></button><button className={`gr-btab-btn ${tab==='more'?'on':''}`} onClick={()=>{navigate('/more/info');}}><I n="more" size={22}/><span>더보기</span></button></div>}
    </div>
  );

  const renderBottomTab = () => (
    <div className="gr-btab"><button className={`gr-btab-btn ${tab==='friends'?'on':''}`} onClick={()=>{navigate('/');}}><I n="user" size={22}/><span>친구</span></button><button className={`gr-btab-btn ${tab==='rooms'?'on':''}`} onClick={()=>{navigate('/calendar');}}><I n="cal" size={22}/><span>캘린더</span></button><button className={`gr-btab-btn ${tab==='alarm'?'on':''}`} onClick={()=>{const now=Date.now();setLastReadAlarm(now);try{localStorage.setItem('gr_last_read_alarm',String(now));}catch(e){}navigate('/alarm');}}><div className="gr-btab-bell-wrap"><I n="bell" size={22}/>{unreadAlarmCount>0&&<span className="gr-btab-badge">{unreadAlarmCount>99?'99+':unreadAlarmCount}</span>}</div><span>알림</span></button><button className={`gr-btab-btn ${tab==='more'?'on':''}`} onClick={()=>{navigate('/more');}}><I n="more" size={22}/><span>더보기</span></button></div>
  );
