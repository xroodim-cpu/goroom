...
function expandRecurring(schedules, year, month) {
  ...
  for (const s of schedules) {
    ...
    if (origDate >= monthStart && origDate <= monthEnd) expanded.push(s);
    if (!s.repeat) continue;

    const { type, interval, endDate } = s.repeat;
    const end = endDate ? new Date(endDate + 'T23:59:59') : new Date(year + 2, 0, 1);
    if (end < monthStart) continue;

    let cur = new Date(origDate);
    const seen = new Set([s.date]);
    for (let i = 0; i < 1000; i++) {
      ...
      if (!seen.has(dateStr)) { seen.add(dateStr); expanded.push({ ...s, date: dateStr, _recurring: true, _origId: s.id }); }
    }
  }
  return expanded;
}

export default function MiniCal({schedules,onSchClick}){
  ...
  return <div style={{flex:1,overflowY:'auto'}}>
    ...
    <div className="gr-cal-sel-info"><div className="gr-cal-sel-date">{sel.getMonth()+1}월 {sel.getDate()}일 {DAYS[sel.getDay()]}요일</div>
      {selSchs.length===0?<div className="gr-cal-empty">스케줄이 없습니다</div>:selSchs.map(sc=> <SchCard key={sc._recurring ? `${sc._origId}-${sc.date}` : sc.id} sc={sc} onClick={()=>onSchClick&&onSchClick(sc)}/>)}
    </div>
  </div>;
}
