...
// TODO: Wasabi 버킷 생성 후 실제 키로 교체
const WASABI_ACCESS_KEY = 'WASABI_ACCESS_KEY';
const WASABI_SECRET_KEY = 'WASABI_SECRET_KEY';
...
/** prefix 기반 폴더 전체 삭제 */
export async function deleteFolderFromWasabi(prefix) {
  try {
    const list = await s3.send(new ListObjectsV2Command({ Bucket: WASABI_BUCKET, Prefix: prefix }));
...
/** prefix로 파일 목록 조회 */
export async function listFromWasabi(prefix) {
  try {
    const list = await s3.send(new ListObjectsV2Command({ Bucket: WASABI_BUCKET, Prefix: prefix }));
